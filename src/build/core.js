...
function optymalizeImageSrc(src, quality) {
  ...
  img.onload = () => {
    ...
    resolve2(newSrc.length < src.length ? newSrc : src);
  };
  img.src = src;
  return promise;
}

// core/build/jsCode.ts
async function jsCode(options) {
...
function filesToString(data, name, type) {
  ...
  return [
    `{`,
    ...Object.keys(data).filter((key) => key !== `type`).reduce((prev, key) => {
...
drawText({ text, ...props });
props.y += 18;
}`, ``).replaceAll(`renderTimer.measure({ Sprite: renderSprite, Text: renderText });`, `renderSprite();renderText();`).replaceAll(`updateTimer.measure({ Physics: updatePhysics, Nodes: updateNodes });`, `updatePhysics();updateNodes();`).replaceAll(`Timer.reset();`, ``).replaceAll(`drawPerformanceInfo();`, ``).split(`
`).filter((line) => !line.includes(`renderTimer`) && !line.includes(`updateTimer`)).join(`
`);
}
